...
import type { Recipe, Product, Donation, ScheduledRecipe, ShoppingListItem, ShoppingList, Ingredient, Note, User, MealType } from './types';
...
// --- ICONS ---
type IconName = 'heart' | 'share' | 'clock' | 'chef-hat' | 'plus' | 'edit' | 'trash' | 'arrow-left' | 'search' | 'star' | 'menu' | 'x' | 'home' | 'pot' | 'cake' | 'sparkles' | 'users' | 'chevron-left' | 'chevron-right' | 'calendar' | 'list' | 'notebook' | 'minus' | 'user' | 'logout' | 'gift' | 'email' | 'youtube' | 'instagram' | 'facebook' | 'check' | 'download' | 'chevron-up' | 'chevron-down' | 'copy' | 'twitter' | 'whatsapp' | 'telegram';
...
const Icons: React.FC<IconProps> = ({ name, className = "w-6 h-6" }) => {
    ...
    const icons: Record<IconName, React.ReactNode> = {
        ...
        'chevron-up': <svg {...iconProps}><path strokeLinecap="round" strokeLinejoin="round" d="m4.5 15.75 7.5-7.5 7.5 7.5" /></svg>,
        'chevron-down': <svg {...iconProps}><path strokeLinecap="round" strokeLinejoin="round" d="m19.5 8.25-7.5 7.5-7.5-7.5" /></svg>,
        ...
        twitter: <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 24 24" className={className}><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>,
        whatsapp: <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 24 24" className={className}><path d="M12.04 2c-5.46 0-9.91 4.45-9.91 9.91 0 1.75.46 3.45 1.32 4.95L2.05 22l5.25-1.38c1.45.79 3.08 1.21 4.74 1.21 5.46 0 9.91-4.45 9.91-9.91S17.5 2 12.04 2zM12.04 20.15c-1.48 0-2.93-.4-4.2-1.15l-.3-.18-3.12.82.83-3.04-.2-.31c-.82-1.31-1.26-2.82-1.26-4.38 0-4.54 3.7-8.24 8.24-8.24 4.54 0 8.24 3.7 8.24 8.24s-3.7 8.24-8.24 8.24zm4.52-6.13c-.25-.12-1.47-.72-1.7-.85-.23-.12-.39-.18-.56.18s-.64.85-.79 1.02c-.14.18-.29.2-.53.06-.25-.12-1.05-.39-2-1.23-.74-.66-1.23-1.47-1.38-1.72s-.02-.38.11-.51c.11-.11.25-.29.37-.43.12-.14.17-.25.25-.41.08-.17.04-.31-.02-.43s-.56-1.34-.76-1.84c-.2-.48-.4-.42-.55-.42-.14 0-.3 0-.46 0s-.39.06-.6.31c-.2.25-.78.76-.78 1.85S8.9 13.6 9.04 13.8c.14.18 1.5 2.29 3.64 3.23.5.21.89.34 1.2.43.48.14.91.12 1.26.08.38-.06 1.17-.48 1.33-. S17.11 15 16.81 14.82c-.29-.17-.53-.25-.78-.37z"/></svg>,
        telegram: <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 24 24" className={className}><path d="M9.78 18.65l.28-4.23 7.68-6.92c.34-.31-.07-.46-.52-.19L7.74 13.3 3.64 12c-.88-.25-.89-.86.2-1.3l15.97-6.16c.73-.33 1.43.18 1.15 1.3l-2.72 12.58c-.28 1.13-1.04 1.4-1.74.88l-4.98-3.9-2.4 2.31c-.26.26-.49.49-.94.49z"/></svg>,
        check: <svg {...iconProps}><path strokeLinecap="round" strokeLinejoin="round" d="M4.5 12.75l6 6 9-13.5" /></svg>,
        download: <svg {...iconProps}><path strokeLinecap="round" strokeLinejoin="round" d="M3 16.5v2.25A2.25 2.25 0 0 0 5.25 21h13.5A2.25 2.25 0 0 0 21 18.75V16.5M16.5 12 12 16.5m0 0L7.5 12m4.5 4.5V3" /></svg>,
        copy: <svg {...iconProps}><path strokeLinecap="round" strokeLinejoin="round" d="M15.666 3.888A2.25 2.25 0 0 0 13.5 2.25h-3c-1.03 0-1.9.693-2.166 1.638m7.332 0c.055.194.084.4.084.612v3.042m-7.416 0a2.25 2.25 0 0 0-2.25 2.25v3.75c0 1.242 1.008 2.25 2.25 2.25h3c1.242 0 2.25-1.008 2.25-2.25v-3.75a2.25 2.25 0 0 0-2.25-2.25h-3.042m0-3.042c-.218-.945-1.136-1.638-2.166-1.638h-3c-1.242 0-2.25 1.008-2.25 2.25v9.75c0 1.242 1.008 2.25 2.25 2.25h9.75c1.242 0 2.25-1.008 2.25-2.25v-9.75a2.25 2.25 0 0 0-2.25-2.25h-3.042" /></svg>,
    };
...
interface AppContextType {
    ...
    scheduledRecipes: ScheduledRecipe[];
    scheduleRecipe: (recipeId: string, date: string, mealType: MealType) => void;
    notes: Note[];
    saveNote: (noteData: { id?: string; title: string; content: string; }) => void;
    deleteNote: (id: string) => void;
    shoppingList: ShoppingList;
    updateShoppingList: (list: ShoppingList) => void;
    deleteShoppingList: (date: string) => void;
    deleteShoppingListItem: (date: string, itemId: number) => void;
    updateShoppingListItem: (date: string, itemId: number, updatedData: { text: string; category: string }) => void;
    completedShoppingLists: string[];
    toggleShoppingListCompletion: (date: string) => void;
...
const Header: React.FC<HeaderProps> = ({ isMenuOpen, setIsMenuOpen }) => {
    ...
    const navLinkClass = ({ isActive }: { isActive: boolean }) =>
      `block py-2 px-3 rounded-lg text-base font-medium transition-colors duration-300 ${isActive ? 'text-brand-primary bg-brand-primary/10' : 'text-brand-text-primary hover:text-brand-primary hover:bg-brand-primary/10'}`;

    const mobileNavLinkClass = ({ isActive }: { isActive: boolean }) =>
      `block w-full text-left p-4 rounded-lg text-lg transition-all duration-300 ${isActive ? 'bg-brand-primary text-white font-bold shadow-lg' : 'text-slate-800 bg-white shadow-sm hover:bg-slate-100 hover:shadow-md'}`;
            ...
            <header className="bg-brand-surface/80 backdrop-blur-sm sticky top-0 z-40 border-b border-slate-200/80">
                <nav className="container mx-auto px-6 py-3">
                    <div className="flex justify-between items-center">
                        <Link to="/" className="text-2xl md:text-3xl font-bold text-brand-primary">
                            Receitas com Charme
                        </Link>
                        <div className="hidden md:flex items-center space-x-2">
                            ...
                            <Link to="/doacoes" className="flex items-center gap-1.5 bg-brand-secondary text-white font-bold py-2 px-4 rounded-full hover:bg-brand-secondary/90 transition-all duration-300 shadow-sm hover:shadow-md transform hover:-translate-y-0.5 ml-4">
                                <Icons name="heart" className="w-5 h-5" />
                                <span>Apoiar</span>
                            </Link>
                        </div>
                        <button className="md:hidden" onClick={() => setIsMenuOpen(true)}>
                            <Icons name="menu" className="w-8 h-8 text-brand-primary" />
                        </button>
                    </div>
                </nav>
            </header>
                        ...
                        >
                            <div className="p-4 flex justify-end">
                                <button onClick={() => setIsMenuOpen(false)}>
                                    <Icons name="x" className="w-8 h-8 text-brand-primary" />
                                </button>
                            </div>
                            ...
                            <div className="px-4 py-6 mt-auto">
                                <Link to="/doacoes" className="block w-full text-center p-4 rounded-xl text-lg bg-gradient-to-r from-brand-secondary to-brand-primary text-white font-bold shadow-lg hover:shadow-xl transform hover:scale-105 transition-all flex items-center justify-center gap-2" onClick={() => setIsMenuOpen(false)}>
...
const BottomNav: React.FC = () => {
    const navLinkClass = ({ isActive }: { isActive: boolean }) =>
        `flex flex-col items-center justify-center text-center w-full transition-all duration-300 ${isActive ? 'text-brand-primary' : 'text-slate-500 hover:text-brand-primary'}`;
        ...
        <nav className="md:hidden fixed bottom-4 left-4 right-4 bg-white/80 backdrop-blur-sm rounded-2xl shadow-2xl z-30 h-16">
            <div className="flex justify-around h-full items-center">
                ...
                <Link to="/adicionar-receita" className="flex items-center justify-center -mt-8">
                    <div className="flex items-center justify-center w-16 h-16 bg-brand-primary rounded-full shadow-lg text-white hover:bg-brand-primary/90 transform hover:scale-105 transition-all duration-300">
...
const Footer: React.FC = () => (
    <footer className="bg-brand-text-primary text-slate-300 mt-16">
        <div className="container mx-auto px-6 py-8 flex flex-col md:flex-row justify-between items-center text-center md:text-left">
            <div>
                <h3 className="text-2xl font-bold text-brand-primary">Receitas com Charme</h3>
                <p className="max-w-md mt-2 text-slate-400">Culinária prática e elegante para o seu dia a dia.</p>
                <div className="mt-4">
                    <Link to="/doacoes" className="bg-brand-secondary text-white font-bold py-2 px-4 rounded-full hover:bg-opacity-90 transition-all duration-300 shadow-md hover:shadow-lg">
                        💛 Apoie o Projeto
                    </Link>
                </div>
            </div>
            <div className="mt-8 md:mt-0">
                ...
                <div className="flex justify-center md:justify-start space-x-4">
                    <a href="https://www.youtube.com/@lojafacilon" target="_blank" rel="noopener noreferrer" className="text-slate-400 hover:text-brand-secondary">YouTube</a>
                    <a href="https://www.instagram.com/lojafacilon" target="_blank" rel="noopener noreferrer" className="text-slate-400 hover:text-brand-secondary">Instagram</a>
                    <a href="https://www.facebook.com/lojafacilon/" target="_blank" rel="noopener noreferrer" className="text-slate-400 hover:text-brand-secondary">Facebook</a>
                    <a href="mailto:receitascomcharme@gmail.com" className="text-slate-400 hover:text-brand-secondary">Email</a>
                </div>
            </div>
        </div>
...
const RecipeCard: React.FC<RecipeCardProps> = ({ recipe }) => {
    ...
    return (
        <div className="bg-brand-surface rounded-2xl shadow-lg overflow-hidden hover:shadow-xl transition-shadow duration-300 group">
            ...
            <div className="p-4">
                <span className="text-xs font-bold text-brand-primary uppercase">{recipe.categoria}</span>
                <h3 className="text-lg font-bold text-brand-text-primary group-hover:text-brand-primary transition-colors duration-300 mt-1 h-14">{recipe.nome}</h3>
    ...
    return (
        <a href={product.link_afiliado} target="_blank" rel="noopener noreferrer" className="bg-brand-surface rounded-2xl shadow-lg overflow-hidden hover:shadow-xl transition-shadow duration-300 group flex flex-col">
            ...
            <div className="p-4 flex flex-col flex-grow">
                 <span className="text-xs font-bold text-brand-primary uppercase">{product.categoria}</span>
                <h3 className="text-lg font-bold text-brand-text-primary group-hover:text-brand-primary transition-colors duration-300 mt-1 flex-1">{product.nome}</h3>
                {product.preco && (
                    <p className="text-lg font-bold text-brand-secondary mt-2">{product.preco}</p>
                )}
                <div className="mt-4">
                    <span className="w-full block text-center bg-brand-primary text-white font-bold py-2 px-4 rounded-full hover:bg-brand-primary/90 transition-all duration-300 shadow-sm group-hover:shadow-md">
                        Ver na Loja
                    </span>
                </div>
            </div>
...
const HomePage: React.FC = () => {
    const { recipes, products, currentUser } = useAppContext();
        ...
        <div className="space-y-16">
            ...
            <section className="relative h-[60vh] flex items-center justify-center text-center text-white bg-slate-900 overflow-hidden p-4">
                ...
                <div className="relative z-10 p-8 animate-fade-in">
                    ...
                    <Link to="/receitas" className="mt-8 inline-block bg-brand-primary text-white font-bold py-3 px-8 rounded-full text-lg hover:bg-brand-primary/90 transition-all duration-300 transform hover:scale-105 shadow-lg hover:shadow-xl">
                        Explorar Receitas
                    </Link>
                </div>
            ...
            {!isPremium && (
                <div className="bg-brand-surface py-2 text-center text-sm text-brand-text-secondary -mt-16">
                    <p>✨ Ative o premium para usar ofline por mais tempo e ter acesso a funcionalidades exclusivas! <Link to="/premium" className="font-bold text-brand-primary underline">Saiba mais</Link>.</p>
                </div>
            )}
            ...
            <section className="container mx-auto px-6">
                 <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6 text-center">
                    <Link to="/receitas?categoria=Culinária Geral" className="group p-6 bg-brand-surface rounded-2xl shadow-lg hover:shadow-xl hover:-translate-y-1 transition-all duration-300 flex flex-col items-center justify-center">
                        <Icons name="pot" className="w-10 h-10 mb-2 text-brand-primary"/>
                    ...
                    <Link to="/receitas?categoria=Pastelaria" className="group p-6 bg-brand-surface rounded-2xl shadow-lg hover:shadow-xl hover:-translate-y-1 transition-all duration-300 flex flex-col items-center justify-center">
                        <Icons name="cake" className="w-10 h-10 mb-2 text-brand-primary"/>
                     ...
                     <Link to="/minhas-receitas" className="group p-6 bg-brand-surface rounded-2xl shadow-lg hover:shadow-xl hover:-translate-y-1 transition-all duration-300 flex flex-col items-center justify-center">
                         <Icons name="chef-hat" className="w-10 h-10 mb-2 text-brand-primary"/>
                    ...
                    <Link to="/favoritas" className="group p-6 bg-brand-surface rounded-2xl shadow-lg hover:shadow-xl hover:-translate-y-1 transition-all duration-300 flex flex-col items-center justify-center">
                        <Icons name="heart" className="w-10 h-10 mb-2 text-brand-primary"/>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
                        {products.slice(0, 8).map(product => (
                            <a href={product.link_afiliado} key={product.id} target="_blank" rel="noopener noreferrer" className="text-center group">
                                ...
                                <p className="text-sm text-brand-secondary font-bold">{product.preco}</p>
                            </a>
                        ))}
                    </div>
                    <div className="text-center mt-12">
                         <Link to="/loja" className="inline-block bg-brand-primary text-white font-bold py-3 px-8 rounded-full text-lg hover:bg-brand-primary/90 transition-all duration-300 transform hover:scale-105 shadow-lg hover:shadow-xl">
            ...
            <section className="container mx-auto px-6">
                <div className="bg-gradient-to-br from-brand-secondary to-brand-primary text-white rounded-2xl p-8 md:p-12 text-center shadow-xl flex flex-col items-center">
                    ...
                    <Link to="/doacoes" className="inline-block bg-white text-brand-primary font-bold py-3 px-8 rounded-full text-lg hover:bg-white/90 transition-all duration-300 transform hover:scale-105 shadow-lg hover:shadow-xl">
                        💛 Fazer uma Doação
                    </Link>
                </div>
            </section>
        ...
        <div className="container mx-auto px-4 py-8">
            ...
            <div className="bg-brand-surface p-4 rounded-2xl shadow-lg mb-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 items-end">
                <div className="w-full">
                    ...
                    <div className="relative">
                        <input
                            ...
                            className="w-full p-2 border border-slate-300 rounded-lg pl-10 focus:ring-2 focus:ring-brand-primary/50 focus:border-brand-primary/80 transition"
                            value={searchTerm}
                ...
                <div>
                    ...
                    <select name="categoria" id="categoria" value={filters.categoria} onChange={handleFilterChange} className="w-full p-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-brand-primary/50 focus:border-brand-primary/80 transition">
                        <option>Todos</option>
                ...
                <div>
                    ...
                    <select name="dificuldade" id="dificuldade" value={filters.dificuldade} onChange={handleFilterChange} className="w-full p-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-brand-primary/50 focus:border-brand-primary/80 transition">
                        <option>Todos</option>
                        <option>Fácil</option>
                        <option>Médio</option>
                ...
                <div>
                    ...
                    <select name="tempo" id="tempo" value={filters.tempo} onChange={handleFilterChange} className="w-full p-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-brand-primary/50 focus:border-brand-primary/80 transition">
                        <option>Todos</option>
        ...
        <AnimatePresence>
            ...
            >
                <motion.div
                    ...
                    transition={{ duration: 0.3 }}
                    className="bg-brand-background rounded-2xl shadow-xl w-full max-w-sm text-center p-8"
                    onClick={e => e.stopPropagation()}
                >
                    <Icons name="sparkles" className="w-16 h-16 text-brand-primary mx-auto mb-4" />
                    <h2 className="text-2xl font-bold text-brand-text-primary mb-2">Funcionalidade Premium</h2>
                    <p className="text-brand-text-secondary mb-6">Esta e outras funcionalidades incríveis estão disponíveis no nosso plano Premium. Faça o upgrade para ter acesso vitalício!</p>
                    <Link to="/premium" onClick={onClose} className="w-full block bg-brand-primary text-white font-bold py-3 px-8 rounded-full text-lg hover:bg-brand-primary/90 transition-all duration-300 shadow-lg hover:shadow-xl">
                        Tornar-se Premium
                    </Link>
                </motion.div>
            </motion.div>
...
const RecipeDetailPage: React.FC = () => {
    const { recipes, products, shoppingList, updateShoppingList, isPremium } = useAppContext();
    const { id } = useParams();
    const navigate = useNavigate();
    ...
    useEffect(() => {
        if (isCookingMode && stepRefs.current[currentStepIndex]) {
            setTimeout(() => {
    ...
    }, [currentStepIndex, isCookingMode]);
    ...
    const handleDownloadTxt = () => {
        ...
        let content = `Receita: ${recipe.nome}\n\n`;
        content += `Categoria: ${recipe.categoria}\n`;
        content += `Tempo: ${recipe.tempo}\n`;
        content += `Porções: ${recipe.porcoes}\n`;
        content += `Dificuldade: ${recipe.dificuldade}\n\n`;
        ...
        recipe.ingredientes.forEach(ing => {
            let line = '- ';
            if (ing.quantidade > 0) {
                line += `${ing.quantidade.toLocaleString('pt-BR')} ${ing.unidade} `;
            }
            if (ing.unidade && ing.unidade !== 'a gosto') {
                line += 'de ';
            }
            line += ing.nome;
        ...
        link.href = url;
        link.download = `${recipe.nome.replace(/\s+/g, '_')}.txt`;
    ...
    const handleDownloadPdf = () => {
        ...
        doc.setFontSize(22);
        doc.text(recipe.nome, 10, 20);

        doc.setFontSize(12);
        doc.text(`Tempo: ${recipe.tempo} | Porções: ${recipe.porcoes} | Dificuldade: ${recipe.dificuldade}`, 10, 30);
        
        autoTable(doc, {
            startY: 40,
            head: [['Quantidade', 'Unidade', 'Ingrediente']],
            body: recipe.ingredientes.map(ing => [ing.quantidade > 0 ? ing.quantidade.toLocaleString('pt-BR') : '', ing.unidade, ing.nome]),
            theme: 'striped',
            headStyles: { fillColor: [231, 76, 60] } // brand-primary color
        });
        
        const lastY = (doc as any).lastAutoTable.finalY;

        doc.setFontSize(16);
        doc.text('Modo de Preparo', 10, lastY + 15);

        doc.setFontSize(12);
        const splitSteps = doc.splitTextToSize(recipe.preparo.map((step, i) => `${i + 1}. ${step}`).join('\n\n'), 180);
        doc.text(splitSteps, 10, lastY + 25);
        
        doc.save(`${recipe.nome.replace(/\s+/g, '_')}.pdf`);
        setDownloadMenuOpen(false);
    };
    ...
    const handleAddToShoppingList = () => {
        ...
        const newItems: ShoppingListItem[] = recipe.ingredientes.map((ing, index) => {
            const adjustedQuantity = ing.quantidade * servingsRatio;
            const textParts = [];
    
            if (adjustedQuantity > 0) {
                textParts.push(adjustedQuantity.toLocaleString('pt-BR', { maximumFractionDigits: 2 }));
            }
            if (ing.unidade) {
                textParts.push(ing.unidade);
            }
            textParts.push(ing.nome);
            ...
            return {
                id: Date.now() + index,
                text: text,
                category: 'Ingredientes da Receita',
                checked: false
            };
        });
        ...
        updatedList[todayKey] = [...todayItems, ...newItems];
    ...
    const handleCopyIngredients = () => {
        if (!recipe) return;
        const ingredientText = recipe.ingredientes.map(ing => {
            const adjustedQuantity = ing.quantidade * servingsRatio;
            let line = '';
            if (adjustedQuantity > 0) {
                line += `${adjustedQuantity.toLocaleString('pt-BR', { maximumFractionDigits: 2 })} ${ing.unidade} de `;
            }
            line += ing.nome;
            return line.trim();
        }).join('\n');
        ...
        navigator.clipboard.writeText(ingredientText).then(() => {
            ...
            alert('Não foi possível copiar os ingredientes.');
        });
    };
    ...
    const handleStepClick = (index: number) => {
        // Can only interact with the current step.
        if (index !== currentStepIndex) {
            return;
        }

        const isCurrentlyChecked = checkedSteps.has(index);

        setCheckedSteps(prevChecked => {
            const newChecked = new Set(prevChecked);
            if (isCurrentlyChecked) {
                // If unchecking, uncheck this and all subsequent steps.
                for (let i = index; i < recipe.preparo.length; i++) {
                    newChecked.delete(i);
                }
            } else {
                // If checking, mark it as complete.
                newChecked.add(index);
            }
            return newChecked;
        });
        
        // Auto-advance if checking the step and it's not the last one
        if (!isCurrentlyChecked && index < recipe.preparo.length - 1) {
            setCurrentStepIndex(index + 1);
        }
    };

    const handleFinishCooking = () => {
        alert('Parabéns! Você concluiu a receita com sucesso.');
        setIsCookingMode(false);
        setCheckedSteps(new Set());
        setCheckedIngredients(new Set());
        setCurrentStepIndex(0);
    ...
    const ShareModal: React.FC = () => {
        ...
        const title = `Receita: ${recipe.nome}`;
        const text = `Confira esta deliciosa receita de ${recipe.nome} que encontrei no Receitas com Charme!`;
        
        const socialLinks = [
            { name: 'WhatsApp', icon: 'whatsapp' as IconName, url: `https://api.whatsapp.com/send?text=${encodeURIComponent(text + ' ' + url)}`, color: 'bg-green-500 hover:bg-green-600' },
            { name: 'Facebook', icon: 'facebook' as IconName, url: `https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(url)}`, color: 'bg-blue-600 hover:bg-blue-700' },
            { name: 'Telegram', icon: 'telegram' as IconName, url: `https://t.me/share/url?url=${encodeURIComponent(url)}&text=${encodeURIComponent(text)}`, color: 'bg-sky-500 hover:bg-sky-600' },
            { name: 'Email', icon: 'email' as IconName, url: `mailto:?subject=${encodeURIComponent(title)}&body=${encodeURIComponent(text + ' ' + url)}`, color: 'bg-slate-500 hover:bg-slate-600' }
        ];
            ...
            <AnimatePresence>
                ...
                >
                    <motion.div
                        ...
                        exit={{ scale: 0.9, opacity: 0, y: 20 }}
                        transition={{ duration: 0.3 }}
                        className="bg-brand-background rounded-2xl shadow-xl w-full max-w-sm"
                        onClick={e => e.stopPropagation()}
                    >
                        <div className="p-6 border-b">
                            <h3 className="text-xl font-bold text-brand-text-primary text-center">Partilhar Receita</h3>
                        </div>
                        <div className="p-6 grid grid-cols-2 gap-4">
                            {socialLinks.map(link => (
                                <a key={link.name} href={link.url} target="_blank" rel="noopener noreferrer" className={`flex items-center justify-center gap-2 text-white font-bold py-3 px-4 rounded-lg transition-colors ${link.color}`}>
                                    <Icons name={link.icon} className="w-5 h-5" />
                                    <span>{link.name}</span>
                                </a>
                            ))}
                        </div>
                        <div className="p-6 border-t">
                             <button onClick={copyLink} className="w-full flex items-center justify-center gap-2 bg-slate-200 text-slate-700 font-bold py-3 px-4 rounded-lg hover:bg-slate-300 transition-colors">
                                <Icons name="copy" className="w-5 h-5"/>
    ...
    return (
        <div className="relative">
                            ...
                            {recipe.imagem_urls.length > 1 && (
                                <div className="grid grid-cols-5 gap-2 p-2">
                                    {recipe.imagem_urls.map((url, index) => (
                                        <img key={index} src={url} alt={`Imagem ${index+1}`} onClick={() => setMainImage(url)} className={`w-full rounded-md cursor-pointer aspect-square object-cover border-4 transition-all ${mainImage === url ? 'border-brand-primary' : 'border-transparent hover:border-brand-primary/50'}`} />
                                    ))}
                                </div>
                            )}
                        </div>
                        ...
                        <div className="lg:col-span-2 p-6">
                            <header className="mb-6 border-b pb-6">
                                <div className="flex justify-between items-start">
                                    <div>
                                        <span className="font-bold text-brand-primary">{recipe.categoria}</span>
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        <div className="relative" ref={downloadMenuRef}>
                                            <button onClick={() => handlePremiumFeature(() => setDownloadMenuOpen(prev => !prev))} className="p-2 rounded-full hover:bg-slate-200">
                                                <Icons name="download" className="w-7 h-7 text-slate-500" />
                                            </button>
                                            <AnimatePresence>
                                            {isDownloadMenuOpen && (
                                                <motion.div
                                                    ...
                                                    className="absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-xl z-20 overflow-hidden border"
                                                >
                                                    <button onClick={handleDownloadPdf} className="w-full text-left px-4 py-3 text-sm text-gray-700 hover:bg-slate-100 transition-colors flex items-center gap-3">
                                                        <Icons name="download" className="w-5 h-5" /> Baixar PDF
                                                    </button>
                                                    <button onClick={handleDownloadTxt} className="w-full text-left px-4 py-3 text-sm text-gray-700 hover:bg-slate-100 transition-colors flex items-center gap-3 border-t">
                                                        <Icons name="download" className="w-5 h-5" /> Baixar TXT
                                                    </button>
                                                </motion.div>
                                            )}
                                            </AnimatePresence>
                                ...
                                <div className="flex items-center text-brand-text-secondary text-md mt-4 space-x-6">
                                    <div className="flex items-center"><Icons name="clock" className="w-5 h-5 mr-2 text-brand-secondary" /><span>{recipe.tempo}</span></div>
                                    <div className="flex items-center"><Icons name="chef-hat" className="w-5 h-5 mr-2 text-brand-secondary" /><span>{recipe.dificuldade}</span></div>
                                    <div className="flex items-center"><Icons name="users" className="w-5 h-5 mr-2 text-brand-secondary" /><span>{recipe.porcoes} porções</span></div>
                                </div>
                            </header>
                            ...
                            <div className="space-y-8">
                                <div>
                                    <div className="flex flex-wrap justify-between items-center mb-4 gap-2">
                                        ...
                                        <div className="flex items-center gap-2">
                                             <button onClick={() => handlePremiumFeature(handleCopyIngredients)} className="flex items-center gap-1.5 text-sm font-bold px-3 py-1.5 rounded-full transition-all duration-300 text-brand-primary bg-brand-primary/10 hover:bg-brand-primary/20" title="Copiar ingredientes (Premium)">
                                                <Icons name="copy" className="w-4 h-4" />
                                                <span>Copiar</span>
                                            </button>
                                            <button 
                                                onClick={handleAddToShoppingList} 
                                                className={`flex items-center gap-1.5 text-sm font-bold px-3 py-1.5 rounded-full transition-all duration-300 ${addedToList ? 'bg-brand-accent text-white' : 'text-brand-primary bg-brand-primary/10 hover:bg-brand-primary/20'}`}
                                                title="Adicionar todos os ingredientes à lista de compras"
                                                disabled={addedToList}
                                            >
                                                {addedToList ? <Icons name="check" className="w-4 h-4" /> : <Icons name="list" className="w-4 h-4" />}
                                                <span>{addedToList ? 'Adicionado!' : 'Adicionar à Lista'}</span>
                                            </button>
                                           
                                        </div>
                                    </div>
                                    <div className="flex items-center gap-2 bg-slate-100 p-1 rounded-full mb-4">
                                        ...
                                        <span className="font-bold text-slate-700 w-full text-center">{servings} {servings > 1 ? "porções" : "porção"}</span>
                                    ...
                                    <ul className="space-y-3 text-brand-text-primary">
                                        {recipe.ingredientes.map((ing, i) => (
                                            <li key={i} onClick={() => isCookingMode && toggleIngredient(i)} className={`transition-all duration-300 ${isCookingMode ? 'flex items-center gap-3 p-2 bg-slate-50 rounded-lg cursor-pointer' : 'pl-2'} ${checkedIngredients.has(i) ? 'opacity-40' : ''}`}>
                                                {isCookingMode && (
                                                    <div className={`w-6 h-6 rounded-md border-2 flex-shrink-0 transition-all ${checkedIngredients.has(i) ? 'bg-brand-accent border-brand-accent' : 'border-slate-300 bg-white'}`}>
                                                ...
                                                <span className={`${checkedIngredients.has(i) ? 'line-through' : ''} ${!isCookingMode ? 'before:content-["•"] before:mr-2 before:text-brand-primary' : ''}`}>
                                ...
                                <div>
                                    ...
                                    <ol className="space-y-4 text-brand-text-primary leading-relaxed">
                                        {recipe.preparo.map((step, i) => {
                                            const isCurrentStep = isCookingMode && i === currentStepIndex;
                                            const isCompleted = checkedSteps.has(i);
                                            return (
                                                <li
                                                    key={i}
                                                    ref={el => { if(el) stepRefs.current[i] = el; }}
                                                    onClick={() => isCookingMode && handleStepClick(i)}
                                                    className={`transition-all duration-500 flex items-start gap-4 ${isCookingMode ? 'p-3 rounded-lg' : ''} ${
                                                        isCurrentStep && !isCompleted ? 'cursor-pointer' : ''
                                                    } ${
                                                        isCompleted
                                                            ? 'bg-brand-accent/10'
                                                            : isCurrentStep
                                                                ? 'bg-brand-primary/10 scale-105 shadow-lg'
                                                                : isCookingMode
                                                                    ? 'opacity-50'
                                                                    : ''
                                                    }`}
                                                >
                                                    <div className={`font-bold mt-1 flex-shrink-0 w-6 h-6 flex items-center justify-center rounded-full ${isCompleted ? 'bg-brand-accent text-white' : isCurrentStep ? 'bg-brand-primary text-white' : 'bg-slate-200 text-slate-600'}`}>
                                                       {isCompleted ? <Icons name="check" className="w-4 h-4" /> : i + 1}
                                                    </div>
                                                    <span className={`${isCompleted ? 'opacity-70 line-through' : ''}`}>{step}</span>
                                                </li>
                                            );
                                        })}
                                    </ol>
                                    {isCookingMode && checkedSteps.size > 0 && checkedSteps.size === recipe.preparo.length && (
                                        <div className="mt-8 text-center animate-fade-in">
                                            <button 
                                                onClick={handleFinishCooking}
                                                className="bg-brand-accent text-white font-bold py-3 px-8 rounded-full text-lg hover:bg-brand-accent/90 transition-all duration-300 transform hover:scale-105 shadow-lg hover:shadow-xl flex items-center gap-2 mx-auto"
                                            >
                                ...
                                <div className="my-8">
                                    <div className="bg-gradient-to-r from-brand-primary/10 to-brand-secondary/10 p-6 rounded-2xl text-center">
                                        ...
                                        <Link to="/doacoes" className="mt-4 inline-block bg-brand-primary text-white font-bold py-2 px-6 rounded-full hover:bg-brand-primary/90 transition-all duration-300 shadow-md hover:shadow-lg">
                                            💛 Apoiar o Projeto
                                        </Link>
                                    </div>
                                </div>
                                ...
                                <div className="flex flex-col gap-4 pt-4">
                                    <button onClick={() => handlePremiumFeature(() => setIsCookingMode(!isCookingMode))} className={`w-full flex items-center justify-center gap-2 font-bold py-3 px-8 rounded-full transition-all duration-300 shadow-md hover:shadow-lg ${isCookingMode ? 'bg-slate-800 text-white' : 'bg-slate-200 text-slate-800'}`}>
                                        <Icons name="pot" className="w-5 h-5"/>
                                        <span>{isCookingMode ? 'Sair do Modo de Cozinha' : 'Cozinhar Passo a Passo'}</span>
                                    </button>
                                     <Link to={`/programar/${recipe.id}`} className="w-full flex items-center justify-center gap-2 bg-brand-secondary text-white font-bold py-3 px-8 rounded-full hover:bg-opacity-90 transition-all duration-300 shadow-md hover:shadow-lg">
                         ...
                         <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                            {products.slice(0, 4).map(product => (
                                <a href={product.link_afiliado} key={product.id} target="_blank" rel="noopener noreferrer" className="text-center group">
                                    ...
                                    <p className="text-xs text-brand-primary group-hover:underline">Ver produto</p>
                                </a>
                            ))}
                        </div>
                    </div>
            ...
            {isCookingMode && (
                <div className="fixed bottom-4 md:bottom-auto md:top-1/2 md:-translate-y-1/2 right-4 z-50 bg-brand-surface/80 backdrop-blur-sm p-3 rounded-2xl shadow-2xl flex md:flex-col items-center gap-3">
                    <button
                        onClick={() => setCurrentStepIndex(prev => Math.max(0, prev - 1))}
                        disabled={currentStepIndex === 0}
                        className="p-3 bg-white rounded-full shadow-lg hover:bg-slate-100 disabled:opacity-50 disabled:cursor-not-allowed transition"
                        aria-label="Passo Anterior"
                    >
                        <Icons name="chevron-up" className="w-6 h-6 text-brand-text-primary" />
                    </button>

                    <div className="font-bold text-brand-text-primary text-sm whitespace-nowrap md:my-2 text-center">
                        Passo <br className="hidden md:block" /> {currentStepIndex + 1}/{recipe.preparo.length}
                    </div>
                </div>
            )}
        </div>
...
const ProfilePage: React.FC = () => {
    const { currentUser, logout, isPremium } = useAppContext();

    const menuItems = [
        { name: 'Minhas Receitas', icon: 'chef-hat', path: '/minhas-receitas', color: 'text-brand-primary' },
        { name: 'Minha Agenda', icon: 'calendar', path: '/agenda', color: 'text-brand-secondary' },
        { name: 'Lista de Compras', icon: 'list', path: '/lista-de-compras', color: 'text-brand-accent' },
        ...
        <div className="container mx-auto px-4 py-8 max-w-2xl">
            ...
            <div className="text-center mb-8">
                ...
                <p className="text-xl text-brand-text-secondary mt-2">Olá, <span className="font-bold">{currentUser?.name}</span>!</p>
                 {isPremium && <p className="mt-2 inline-flex items-center gap-2 text-amber-700 font-bold bg-amber-100 px-3 py-1 rounded-full"><Icons name="star" className="w-5 h-5"/> Membro Premium</p>}
            </div>
            
             {!isPremium && (
                <div className="mb-8">
                    <Link to="/premium" className="block bg-gradient-to-r from-brand-primary to-brand-secondary text-white p-6 rounded-2xl shadow-lg hover:shadow-xl transition-shadow text-center">
                        <h2 className="text-2xl font-bold">✨ Desbloqueie o Premium</h2>
                        <p className="mt-1">Acesso vitalício a funcionalidades exclusivas com um pagamento único!</p>
                    </Link>
                </div>
             )}
    ...
    return (
        <div className="container mx-auto px-4 py-8">
            ...
            <div className="flex justify-between items-center mb-8">
                ...
                <Link to="/adicionar-receita" className="hidden md:flex items-center gap-2 bg-brand-primary text-white font-bold py-2 px-4 rounded-full hover:bg-brand-primary/90 transition-all duration-300 shadow-md hover:shadow-lg">
            ...
            {myRecipes.length > 0 ? (
                ...
                <div className="text-center py-16 bg-brand-surface rounded-2xl shadow-lg">
                    ...
                    <Link to="/adicionar-receita" className="mt-4 inline-block bg-brand-primary text-white font-bold py-3 px-6 rounded-full hover:bg-brand-primary/90 transition-all duration-300 shadow-md hover:shadow-lg">
        ...
        <div className="container mx-auto px-4 py-8 max-w-3xl">
            ...
            <form onSubmit={handleSubmit} className="bg-brand-surface p-6 rounded-2xl shadow-lg space-y-6">
                ...
                <div>
                    ...
                    <button type="button" onClick={addIngredient} className="mt-2 text-sm font-bold text-brand-primary hover:underline">
                ...
                <div className="text-right">
                    <button type="submit" className="bg-brand-primary text-white font-bold py-3 px-8 rounded-full hover:bg-brand-primary/90 transition-all duration-300 shadow-md hover:shadow-lg">{buttonText}</button>
                </div>
            </form>
        </div>
    );
...
const ShoppingListPage: React.FC = () => {
    const { shoppingList, updateShoppingList, completedShoppingLists, toggleShoppingListCompletion, deleteShoppingList, deleteShoppingListItem, updateShoppingListItem } = useAppContext();
    ...
    const handleDeleteItem = (date: string, itemId: number) => {
        if (window.confirm('Tem certeza de que deseja excluir este item?')) {
            deleteShoppingListItem(date, itemId);
        }
    };

    const handleUpdateItem = () => {
        if (!editingItem || !selectedDate) return;
        updateShoppingListItem(selectedDate, editingItem.id, { text: editingItem.text, category: editingItem.category });
        setEditingItem(null);
    };

    const handleShareShoppingList = (date: string, items: ShoppingListItem[] | undefined) => {
        ...
        let shareText = `Lista de Compras - ${new Date(date).toLocaleDateString('pt-BR', { timeZone: 'UTC' })}\n\n`;

        Object.keys(groupedItems).sort().forEach(category => {
            shareText += `*${category}*\n`;
            groupedItems[category].forEach(item => {
                const checkedMark = item.checked ? '[x]' : '[ ]';
                shareText += `${checkedMark} ${item.text}\n`;
            });
            shareText += '\n';
        });
        
        shareText += "Criado com o app Receitas com Charme!";

        if (navigator.share) {
            navigator.share({
                title: `Minha Lista de Compras - ${new Date(date).toLocaleDateString('pt-BR', { timeZone: 'UTC' })}`,
                text: shareText,
    ...
    const categories = ['Ingredientes da Receita', 'Frutas', 'Verduras', 'Carne', 'Laticínios', 'Padaria', 'Bebidas', 'Limpeza', 'Outros'];
        ...
        <div className="container mx-auto px-4 py-8 max-w-2xl">
            ...
            <form onSubmit={handleAddItem} className="bg-brand-surface p-4 rounded-2xl shadow-lg mb-8 space-y-4">
                 ...
                 <input
                    ...
                    className="w-full p-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-brand-primary/50 focus:border-brand-primary/80 transition"
                />
                <div className="grid grid-cols-2 gap-4">
                    ...
                    <button type="submit" className="w-full bg-brand-primary text-white font-bold py-3 px-4 rounded-lg hover:bg-brand-primary/90 transition-all shadow-md">Adicionar</button>
                </div>
            </form>
             ...
             {sortedDates.length > 0 ? (
                <div className="space-y-3">
                    {sortedDates.map(date => {
                        const isCompleted = completedShoppingLists.includes(date);
                        return (
                             <button key={date} onClick={() => setSelectedDate(date)} className={`w-full text-left p-4 rounded-lg shadow-md hover:shadow-lg transition-all flex justify-between items-center ${isCompleted ? 'bg-green-50 opacity-70' : 'bg-brand-surface'}`}>
                                <div className="flex items-center gap-3">
                                    {isCompleted && <Icons name="check" className="w-5 h-5 text-brand-accent" />}
                                    <span className={`font-bold ${isCompleted ? 'text-gray-500 line-through' : 'text-brand-text-primary'}`}>
                     ...
                     >
                        ...
                        >
                            ...
                            <div className="p-4 overflow-y-auto">
                               {Object.keys(groupedItems).length > 0 ? Object.keys(groupedItems).sort().map(category => (
                                    <div key={category} className="mb-4">
                                        <h3 className="font-bold text-brand-primary mb-2">{category}</h3>
                                        <ul className="space-y-2">
                                            {groupedItems[category].map(item => (
                                                <li key={item.id} className={`group flex items-center gap-3 p-2 rounded-lg ${editingItem?.id === item.id ? 'bg-slate-200' : (item.checked ? 'bg-slate-100' : 'bg-brand-surface')}`}>
                                                    {editingItem?.id === item.id ? (
                                                        <div className="w-full space-y-2">
                                                            ...
                                                            <div className="flex justify-end gap-2 mt-2">
                                                                <button onClick={() => setEditingItem(null)} className="px-3 py-1 text-sm rounded-md bg-slate-300 hover:bg-slate-400">Cancelar</button>
                                                                <button onClick={handleUpdateItem} className="px-3 py-1 text-sm rounded-md bg-brand-primary text-white hover:bg-brand-primary/90">Salvar</button>
                                                            </div>
                                                        </div>
                                                    ) : (
                                                        <>
                                                            <div onClick={() => toggleItem(selectedDate, item.id)} className="flex-grow flex items-center gap-3 cursor-pointer">
                                                                <div className={`w-5 h-5 rounded border-2 flex-shrink-0 ${item.checked ? 'bg-brand-accent border-brand-accent' : 'border-slate-400'}`}>
                                                            ...
                                                            <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                                                <button onClick={() => setEditingItem({ ...item })} className="p-1 rounded-full hover:bg-slate-200"><Icons name="edit" className="w-4 h-4 text-slate-600" /></button>
                                                                <button onClick={() => handleDeleteItem(selectedDate, item.id)} className="p-1 rounded-full hover:bg-red-100"><Icons name="trash" className="w-4 h-4 text-red-500" /></button>
                                                            </div>
                                                        </>
                                                    )}
                                                </li>
                                ...
                                )) : <p className="text-center text-brand-text-secondary">Esta lista de compras está vazia.</p>}
                            </div>
                            <div className="p-4 mt-auto border-t grid grid-cols-3 gap-2">
                                <button onClick={() => handleShareShoppingList(selectedDate, itemsForSelectedDate)} className="w-full flex items-center justify-center gap-2 bg-blue-100 text-blue-700 font-bold py-2 px-4 rounded-lg hover:bg-blue-200 transition-colors">
                                    <Icons name="share" className="w-5 h-5"/> Compartilhar
                                </button>
                                <button onClick={() => toggleShoppingListCompletion(selectedDate)} className="w-full flex items-center justify-center gap-2 bg-green-100 text-green-700 font-bold py-2 px-4 rounded-lg hover:bg-green-200 transition-colors">
                                    <Icons name="check" className="w-5 h-5"/> {completedShoppingLists.includes(selectedDate) ? 'Desmarcar' : 'Concluída'}
                                </button>
                                <button onClick={() => {
                                    if (window.confirm('Tem certeza que deseja excluir esta lista? A ação não pode ser desfeita.')) {
                                        deleteShoppingList(selectedDate);
                                        setSelectedDate(null);
                                    }
                                }} className="w-full flex items-center justify-center gap-2 bg-red-100 text-red-700 font-bold py-2 px-4 rounded-lg hover:bg-red-200 transition-colors">
                                    <Icons name="trash" className="w-5 h-5"/> Excluir
                                </button>
                            </div>
                        </motion.div>
                     </motion.div>
        ...
        <div className="container mx-auto px-4 py-8 max-w-3xl">
            ...
            {Object.keys(groupedNotes).length > 0 ? (
                ...
                <div className="text-center py-16 bg-brand-surface rounded-2xl shadow-lg">
                    ...
                    <Link to="/bloco-de-notas/nova" className="mt-4 inline-block bg-brand-primary text-white font-bold py-3 px-6 rounded-full hover:bg-brand-primary/90 transition-all duration-300 shadow-md hover:shadow-lg">
                        Criar primeira anotação
                    </Link>
                </div>
            )}

            <Link to="/bloco-de-notas/nova" className="fixed bottom-24 right-6 md:bottom-8 md:right-8 w-16 h-16 bg-brand-primary rounded-full shadow-lg text-white flex items-center justify-center hover:bg-brand-primary/90 transform hover:scale-105 transition-all duration-300 z-30">
                <Icons name="plus" className="w-8 h-8" />
            </Link>
        </div>
    );
        ...
        <div className="container mx-auto px-4 py-8 max-w-3xl">
            ...
            <form onSubmit={handleSubmit} className="bg-brand-surface p-6 rounded-2xl shadow-lg space-y-6">
                ...
                <div className="flex justify-between items-center pt-4 border-t">
                    ...
                    <button
                        type="submit"
                        className="bg-brand-primary text-white font-bold py-3 px-8 rounded-full hover:bg-brand-primary/90 transition-all duration-300 shadow-md hover:shadow-lg"
                    >
                       Salvar Anotação
                    </button>
                </div>
...
const ScheduleRecipePage: React.FC = () => {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        scheduleRecipe(recipe.id, date, mealType);
    ...
    return (
        <div className="container mx-auto px-4 py-8 max-w-lg">
            ...
            <form onSubmit={handleSubmit} className="bg-brand-surface p-6 rounded-2xl shadow-lg space-y-4">
                <div>
                    <label htmlFor="date" className="block text-lg font-bold text-brand-text-primary mb-2">Selecione a data:</label>
                    <input
                        type="date"
                        id="date"
                        value={date}
                ...
                <div>
                    <label htmlFor="mealType" className="block text-lg font-bold text-brand-text-primary mb-2">Selecione a refeição:</label>
                    <select
                        id="mealType"
                        value={mealType}
                ...
                <button type="submit" className="w-full bg-brand-primary text-white font-bold py-3 px-8 rounded-full hover:bg-brand-primary/90 transition-all duration-300 shadow-md hover:shadow-lg !mt-6">Agendar</button>
            </form>
        </div>
    );
};
...
const SchedulePage: React.FC = () => {
    ...
    const scheduledByDate = useMemo(() => {
        const grouped: Record<string, ScheduledRecipe[]> = {};
        ...
        sorted.forEach(scheduled => {
            ...
            grouped[scheduled.date].push(scheduled);
        });
        return grouped;
    }, [scheduledRecipes]);

    const mealOrder: MealType[] = ['Café da manhã', 'Almoço', 'Lanche', 'Jantar'];
    ...
    return (
        <div className="container mx-auto px-4 py-8">
            ...
            {Object.keys(scheduledByDate).length > 0 ? (
                <div className="space-y-8">
                    {Object.entries(scheduledByDate).map(([date, scheduledItems]) => {
                        const recipesByMeal = mealOrder.map(meal => ({
                            mealType: meal,
                        ...
                        return (
                            <div key={date}>
                                ...
                                {recipesByMeal.map(({ mealType, recipes: recipesOnMeal }) => (
                                    <div key={mealType} className="mb-6">
                                        <h3 className="text-lg font-bold text-brand-primary mb-3">{mealType}</h3>
                                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                                            {recipesOnMeal.map(recipe => (
        ...
        <div className="container mx-auto px-4 py-8 max-w-4xl">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {donations.map((donation) => (
                    <div key={donation.metodo} className="bg-brand-surface rounded-2xl shadow-lg p-6 text-center flex flex-col items-center border-t-4 border-brand-primary hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
                        <div className="p-4 bg-brand-primary/10 rounded-full mb-4">
                            <Icons name="gift" className="w-10 h-10 text-brand-primary" />
                        </div>
                        ...
                        <a 
                            ...
                            className="mt-auto w-full bg-brand-primary text-white font-bold py-3 px-6 rounded-full hover:bg-brand-primary/90 transition-all duration-300 shadow-md hover:shadow-lg transform hover:scale-105"
                        >
                            Contribuir Agora
                        </a>
                    </div>
            ...
            <div className="flex justify-center flex-wrap gap-2 mb-8">
                {categories.map(category => (
                    <button
                        ...
                        className={`px-4 py-2 font-bold rounded-full transition-colors duration-300 ${selectedCategory === category ? 'bg-brand-primary text-white shadow-md' : 'bg-brand-surface text-brand-text-secondary hover:bg-slate-200'}`}
                    >
                        {category}
                    </button>
                ))}
...
const GoPremiumPageLock: React.FC<{ title: string }> = ({ title }) => (
    <div className="container mx-auto px-4 py-8 max-w-2xl text-center">
        <BackButton className="mb-4" />
        <div className="bg-brand-surface p-8 rounded-2xl shadow-lg">
            <Icons name="sparkles" className="w-16 h-16 text-brand-primary mx-auto mb-4" />
            <h1 className="text-3xl font-bold text-brand-text-primary mb-2">Recurso Premium: {title}</h1>
            <p className="text-brand-text-secondary mb-6">Esta funcionalidade está disponível apenas para membros Premium. Desbloqueie agora para ter acesso instantâneo!</p>
            <Link to="/premium" className="inline-block bg-brand-primary text-white font-bold py-3 px-8 rounded-full text-lg hover:bg-brand-primary/90 transition-all duration-300 transform hover:scale-105 shadow-lg hover:shadow-xl">
                Tornar-se Premium
            </Link>
        </div>
    </div>
...
const SuggestionPage: React.FC = () => {
    const { saveRecipe, toggleFavorite, isFavorite, isPremium } = useAppContext();
    ...
    }, [fetchSuggestion, isPremium]);
    
    if (!isPremium) {
        return <GoPremiumPageLock title="Sugestão do Dia" />;
    }
        ...
        <div className="container mx-auto px-4 py-8 max-w-3xl">
            ...
            {suggestion && (
                <div className="bg-brand-surface p-8 rounded-2xl shadow-lg animate-fade-in text-brand-text-primary leading-relaxed">
                    <h2 className="text-3xl font-bold text-brand-primary mb-2 text-center">{suggestion.nome}</h2>
                    <div className="flex items-center justify-center text-brand-text-secondary text-sm mb-4 space-x-4">
                        <div className="flex items-center"><Icons name="clock" className="w-4 h-4 mr-1.5 text-brand-secondary" /><span>{suggestion.tempo}</span></div>
                        <div className="flex items-center"><Icons name="chef-hat" className="w-4 h-4 mr-1.5 text-brand-secondary" /><span>{suggestion.dificuldade}</span></div>
                        <div className="flex items-center"><Icons name="users" className="w-4 h-4 mr-1.5 text-brand-secondary" /><span>{suggestion.porcoes} porções</span></div>
                    </div>
                    <p className="text-brand-text-secondary mb-6 italic text-center">{suggestion.descricao}</p>
                    
                    <div className="grid md:grid-cols-2 gap-8">
                        <div>
                            <h3 className="text-2xl font-bold mb-3 border-b-2 border-brand-primary/50 pb-2">Ingredientes</h3>
                            <ul className="space-y-2 mt-4">
                                {suggestion.ingredientes.map((ing, i) => <li key={i} className="flex items-start gap-2"><span className="text-brand-primary font-bold mt-1.5">•</span><span>{ing.quantidade > 0 && <strong>{ing.quantidade.toLocaleString('pt-BR')} {ing.unidade}</strong>} {ing.unidade !== 'a gosto' && ing.unidade && ' de '} {ing.nome}</span></li>)}
                            </ul>
                        </div>

                        <div>
                            <h3 className="text-2xl font-bold mb-3 border-b-2 border-brand-primary/50 pb-2">Modo de Preparo</h3>
                            <ol className="space-y-4 mt-4">
                                {suggestion.preparo.map((step, i) => <li key={i} className="flex items-start gap-3"><span className="font-bold text-brand-primary mt-1">{i + 1}.</span><span>{step}</span></li>)}
                            </ol>
                        </div>
                    </div>
                     ...
                     <div className="mt-8 pt-6 border-t border-slate-200 flex flex-col sm:flex-row items-center justify-center gap-4">
                        <button
                            ...
                            className="w-full sm:w-auto flex items-center justify-center gap-2 bg-brand-secondary text-white font-bold py-3 px-6 rounded-full hover:bg-opacity-90 transition-all duration-300 shadow-md hover:shadow-lg disabled:bg-slate-400 disabled:cursor-not-allowed"
                        >
            ...
            {!loading && (
                <div className="text-center mt-8">
                    <button 
                        ...
                        disabled={loading}
                        className="bg-brand-primary text-white font-bold py-3 px-8 rounded-full hover:bg-brand-primary/90 transition-all duration-300 shadow-md hover:shadow-lg disabled:bg-slate-400 disabled:cursor-not-allowed flex items-center gap-2 mx-auto"
                    >
...
const PremiumPage: React.FC = () => {
    const { isPremium, activatePremium } = useAppContext();
    ...
    const handleActivation = (e: React.FormEvent) => {
        e.preventDefault();
        setError('');
        if (coupon.toLowerCase() === 'rcc') {
            activatePremium();
            alert('Premium ativado com sucesso! Aproveite todos os benefícios.');
            navigate('/perfil');
        } else {
            setError('Código de ativação inválido. Tente novamente.');
        }
    };
    
    const premiumFeatures = [
        { icon: 'sparkles' as IconName, text: 'Sugestões de receitas diárias via IA' },
        { icon: 'pot' as IconName, text: 'Modo de Cozinha interativo passo a passo' },
        { icon: 'download' as IconName, text: 'Download de receitas em PDF e TXT' },
        { icon: 'copy' as IconName, text: 'Copiar lista de ingredientes com 1 clique' },
        { icon: 'list' as IconName, text: 'Suporte Offline Aprimorado' },
    ];
    ...
    if (isPremium) {
        return (
            <div className="container mx-auto px-4 py-8 max-w-2xl text-center">
                <div className="bg-brand-surface p-8 rounded-2xl shadow-lg">
                    <Icons name="star" className="w-16 h-16 text-yellow-500 mx-auto mb-4" />
                    <h1 className="text-3xl font-bold text-brand-text-primary mb-2">Você já é Premium!</h1>
                    <p className="text-brand-text-secondary mb-6">Obrigado por apoiar nosso projeto. Você tem acesso a todas as funcionalidades exclusivas.</p>
                    <Link to="/perfil" className="inline-block bg-brand-primary text-white font-bold py-3 px-8 rounded-full text-lg hover:bg-brand-primary/90 transition-all duration-300 shadow-lg">
                        Voltar ao Perfil
                    </Link>
                </div>
            </div>
    ...
    return (
        <div className="container mx-auto px-4 py-8 max-w-2xl">
            <BackButton className="mb-4" />
            <div className="bg-brand-surface p-8 rounded-2xl shadow-2xl">
                <div className="text-center">
                    <Icons name="sparkles" className="w-16 h-16 text-brand-primary mx-auto mb-4" />
                    <h1 className="text-4xl font-bold text-brand-text-primary">Seja um Membro Premium</h1>
                    <p className="text-brand-text-secondary mt-2">Acesso vitalício a funcionalidades exclusivas com um pagamento único.</p>
                </div>

                <div className="my-8">
                    <h2 className="text-xl font-bold text-center mb-4">O que você ganha:</h2>
                    <ul className="space-y-3">
                        {premiumFeatures.map(feature => (
                            <li key={feature.text} className="flex items-center gap-3">
                                <div className="p-2 bg-brand-primary/10 rounded-full">
                                    <Icons name={feature.icon} className="w-5 h-5 text-brand-primary" />
                                </div>
                                <span className="text-brand-text-primary">{feature.text}</span>
                            </li>
                        ))}
                    </ul>
                </div>

                <div className="text-center p-6 bg-brand-primary/5 rounded-lg">
                    <h3 className="text-lg font-bold">Pagamento Único</h3>
                    <p className="text-4xl font-extrabold my-2 text-brand-primary">$2.99 USD</p>
                    <p className="text-brand-text-secondary text-sm">Pagamento único para acesso vitalício.</p>
                    <a 
                        href="https://www.paypal.com/ncp/payment/BVKP3NG583USG"
                        target="_blank"
                        rel="noopener noreferrer"
                        className="mt-4 w-full block bg-brand-primary text-white font-bold py-3 px-8 rounded-full text-lg hover:bg-brand-primary/90 transition-all duration-300 shadow-lg hover:shadow-xl"
                    >
                        Pagar com PayPal
                    </a>
                </div>
                ...
                <div className="mt-8 pt-6 border-t">
                     <h2 className="text-xl font-bold text-center mb-2">Já efetuou o pagamento?</h2>
                     <p className="text-center text-brand-text-secondary mb-4">Use o código para ativar seu acesso vitalício.</p>
                     <form onSubmit={handleActivation} className="flex flex-col gap-2 items-center">
                        <p className="text-sm text-center font-semibold text-brand-text-secondary">Escreva <strong className="text-brand-primary">rcc</strong> para ser premium para sempre.</p>
                        <input 
                            type="text"
                            value={coupon}
                            onChange={(e) => setCoupon(e.target.value)}
                            className="w-full max-w-sm p-3 text-center border border-slate-300 rounded-lg focus:ring-2 focus:ring-brand-primary/50"
                            placeholder="Seu código de ativação"
                        />
                        {error && <p className="text-red-500 text-sm mt-1">{error}</p>}
                        <button type="submit" className="mt-2 w-full max-w-sm bg-brand-accent text-white font-bold py-3 px-6 rounded-full hover:bg-brand-accent/90 transition-all duration-300 shadow-md">
                            Ativar Premium
                        </button>
                     </form>
                </div>
    ...
    return (
        <div className="min-h-screen bg-brand-background flex items-center justify-center p-4">
            <div className="w-full max-w-md mx-auto">
                <div className="text-center mb-8">
                    <Icons name="chef-hat" className="mx-auto w-16 h-16 text-brand-primary" />
                        ...
                        <div className="space-y-3 mb-6">
                            ...
                            {users.map(user => (
                                <button
                                    ...
                                    className="w-full flex items-center gap-4 p-4 bg-slate-100 rounded-lg text-left hover:bg-brand-primary/10 hover:text-brand-primary transition-colors"
                                >
                                    <Icons name="user" className="w-6 h-6 text-brand-primary" />
                    ...
                    <div>
                        ...
                        <form onSubmit={handleCreateUser} className="flex gap-2">
                            <input
                                ...
                                className="flex-grow p-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-brand-primary/50"
                                required
                            />
                            <button
                                type="submit"
                                className="bg-brand-primary text-white font-bold p-3 rounded-lg hover:bg-brand-primary/90 transition-colors shadow"
                            >
                                Criar
                            </button>
                        </form>
...
const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    const createUser = (name: string) => {
        ...
        const newUser: User = { id: newUserId, name, isPremium: false };
    ...
    const currentUserData = currentUser ? allData.userData[currentUser.id] || { favorites: [], scheduledRecipes: [], notes: [], shoppingList: {}, recipes: [], completedShoppingLists: [], isPremium: false } : { favorites: [], scheduledRecipes: [], notes: [], shoppingList: {}, recipes: [], completedShoppingLists: [], isPremium: false };
    ...
    const activatePremium = () => {
        ...
        const updatedUsers = allData.users.map(u => 
            u.id === currentUser.id ? { ...u, isPremium: true } : u
        );
        ...
        setAllData(prev => ({
            ...
            userData: {
                ...prev.userData,
                [currentUser.id]: {
                    ...(prev.userData[currentUser.id] || currentUserData),
                    isPremium: true,
                }
            }
        }));
    ...
    const scheduleRecipe = (recipeId: string, date: string, mealType: MealType) => {
        const newScheduled: ScheduledRecipe = { id: Date.now().toString(), recipeId, date, mealType };
    ...
    const toggleShoppingListCompletion = (date: string) => {
        if (!currentUser) return;
        const currentCompleted = currentUserData.completedShoppingLists || [];
        const isCompleted = currentCompleted.includes(date);
        const newCompleted = isCompleted
            ? currentCompleted.filter(d => d !== date)
            : [...currentCompleted, date];
        updateUserData({ completedShoppingLists: newCompleted });
    };

    const deleteShoppingList = (date: string) => {
        if (!currentUser) return;
        const updatedList = { ...currentUserData.shoppingList };
        delete updatedList[date];

        const currentCompleted = currentUserData.completedShoppingLists || [];
        const newCompleted = currentCompleted.filter(d => d !== date);

        updateUserData({
            shoppingList: updatedList,
            completedShoppingLists: newCompleted
        });
    };
    
    const deleteShoppingListItem = (date: string, itemId: number) => {
        if (!currentUser) return;
        const currentList = currentUserData.shoppingList[date];
        if (!currentList) return;
    
        const newListForDate = currentList.filter(item => item.id !== itemId);
        
        const updatedShoppingList = {
            ...currentUserData.shoppingList,
            [date]: newListForDate
        };
    
        if (newListForDate.length === 0) {
            delete updatedShoppingList[date];
        }
        
        updateUserData({ shoppingList: updatedShoppingList });
    };
    
    const updateShoppingListItem = (date: string, itemId: number, updatedData: { text: string; category: string }) => {
        if (!currentUser) return;
        const currentList = currentUserData.shoppingList[date];
        if (!currentList) return;
        
        const newListForDate = currentList.map(item =>
    ...
    const contextValue: AppContextType = {
        ...
        completedShoppingLists: currentUserData.completedShoppingLists || [],
        toggleShoppingListCompletion,
        saveRecipe,
        deleteRecipe,
    };
...
const App: React.FC = () => {
    ...
    return (
        <div className="flex flex-col min-h-screen bg-brand-background text-brand-text-primary font-sans">
            <Header isMenuOpen={isMenuOpen} setIsMenuOpen={setIsMenuOpen} />
            <main className="flex-grow pt-8 pb-24 md:pb-8">
                 <AnimatePresence mode="wait">
                    <Routes location={location} key={location.pathname}>
                        ...
                        <Route path="/premium" element={<PageWrapper><PremiumPage /></PageWrapper>} />
                    </Routes>
                </AnimatePresence>
            </main>
            <Footer />
